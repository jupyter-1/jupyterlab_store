...
export class KernelModel {
  ...
  requestExecute(
    ...
    this.future = this._sessionContext.session?.kernel?.requestExecute(content);
    return this.future.done.then(res => {
      console.log('[KERNEL] execute done', res);
      setTimeout(() => this._sessionContext.session?.shutdown(), 5000);
      return res;
    });
  }
